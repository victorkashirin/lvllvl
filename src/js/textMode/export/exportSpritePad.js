...
ExportSpritePad.prototype = {
  ...
  start: function() {
    ...
    if(this.uiComponent == null) {
      this.uiComponent = UI.create("UI.Dialog", { "id": "exportSpritePadDialog", "title": "Export SpritePad", "width": 300, "height": 100 });

      this.htmlComponent = UI.create("UI.HTMLPanel");
      this.uiComponent.add(this.htmlComponent);
      this.htmlComponent.load('html/textMode/exportSpritePad.html', function() {
        _this.initContent();
        _this.initEvents();
      });
    ...
    UI.showDialog("exportSpritePadDialog");
  },  
  ...
  exportSpritePadAs: function(args) {
    ...
    if(blockModeEnabled) {
      blockSet = layer.getCurrentBlockSet();
      if(blockSet) {
...
//      for(var y = mapHeight - 1; y >= 0; y--) {
    ...
    var spritePadData = new Uint8Array(data.length);
    for(var i = 0; i < spritePadData.length; i++) {
      spritePadData[i] = data[i];
    }

    var filename = args.filename + '.ctm';
    download(spritePadData, filename, "application/ctm");

  },

  exportSpritePad: function() {
    var args = {};
    args.filename = $('#exportSpritePadAs').val();

    console.log('export sprite pad');
    this.exportSpritePadAs(args);
  }  
}
