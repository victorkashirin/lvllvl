...
//    $('#c64PRGCodeEditorResult').val(result);
    if(result.success) {
...
//      $('#c64PRGCodeEditorResult').val(out);
  ...
  createPRG: function(args) {
    ...
    // input data is an array of frame data
    ...
    for(var i = 0; i < this.blocks.length; i++) {
      blocks.push({ 
        "type": "Code", 
        "start": this.blocks[i].start, 
        "end": this.blocks[i].end, 
        "label": this.blocks[i].label 
      });
    }
    ...
    */
    ...
    // the base prg (created by assembleCode function)
    var prg = this.prg;
    ...
    console.log("CODE FROM TO = " + dataFrom + ',' + dataTo);
...
//          this.editor.tileSets[i].memoryLocation = (7 - charBlock) << 1;
    ...
    // these overwrite the config values in the asm..
...
//      console.log(i + ':' + frameTableAddr.toString(16));
...
//            console.log('frame' + )
//            console.log('frame type = ' + frameData.charFrameType);
            // char data entry
...
//              console.log('table entry for frame  ' + frame);
    ...
    this.displayMemoryLayout(blocks);
  ...
  displayMemoryLayout: function(blocks) {
