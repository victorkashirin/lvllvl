...
PatternView2.prototype = {
  ...
  setMode: function(mode) {
    ...
    $('.musicTool').removeClass('musicToolSelected');
    $('#musicTool_' + mode).addClass('musicToolSelected');
...
//    this.initEffectDialog();
  ...
  selectNote: function(noteId) {
    ...
    if(noteId === false) {
      console.error('note id is false!');
    }
  ...
  selectAll: function() {
    ...
    this.music.patterns.selectAllNotes(this.patternId);
    this.drawPattern();
  },
  ...
  patternLengthUpdated: function() {
    this.unselectAllNotes();
    var patternDuration = this.music.patterns.getDuration(this.patternId);
    var patternName = this.music.patterns.getName(this.patternId);
    ...
    this.setYScroll(this.scrollY );
    this.setXScroll(this.scrollX );
  ...
  keyDown: function(event) {
    ...
    if(note != -1 && note != this.keyboardNote) {
      ...
      if(this.recordMode) {
        ...
        // key down add note
        this.music.doc.data.patterns[this.patternId].addNote(position, 
                                                    instrumentId, 
                                                    pitch, 
                                                    length);
        ...
        if(legato) {
          this.music.patterns.addParam(this.patternId, "effects", position, 
                                      { "effect": 17, "value1": 0, "value2": 0 } );
        }
        this.drawPattern();
...
//          this.updatePattern();
  ...
  mouseDownGridSelect: function(button, x, y) {
    ...
    this.currentNoteId = this.gridToNoteId(grid.x, grid.y);

    if(this.currentNoteId !== false) {

      // mouse is over a note
      var pattern = this.music.doc.data.patterns[this.patternId];
      this.dragOffsetX = 0;
      this.dragOffsetY = 0;

      if(!this.music.patterns.getNoteSelected(this.patternId, this.currentNoteId)) {
...
//        this.music.history.startPatternChange(this.patternId);
        ...
        this.music.history.startEntry('addNote');
        ...
        if(legato) {
          this.music.patterns.addParam(this.patternId, "effects", position, { "effect": 17, "value1": 0, "value2": 0 } );

          this.editor.history.addAction('setParam', { 
  ...
  showContextMenu: function(event) {
    ...
    if(this.patternViewPopup == null) {
      ...
      this.patternViewPopup.init(this.music, function() {
        ...
        _this.patternViewPopup.show({ "selectedInstrument": selectedInstrument, x: x, y: y });
      });
    } else {
      this.patternViewPopup.show({ "selectedInstrument": selectedInstrument, x: x, y: y });
    }
  },
...
//    var factor = 1;
 ...
 //   factor = event.deltaFactor / 6;
  ...
  selectNotes: function() {
    ...
    var selectTop = Math.floor( (this.selectTop - this.hScrollBarHeight) / this.cellHeight);
    var selectBottom = Math.floor( (this.selectBottom - this.hScrollBarHeight) / this.cellHeight);
    ...
    this.music.patterns.selectNotesIn(this.patternId, selectFrom, selectTo, selectBottom, selectTop);
    return;

  },

  // find the position of the first selected note, needed for limiting dragging
  setFirstSelectedNotePosition: function() {
    var notes = this.music.patterns.getNotes(this.patternId);

    // set it to max
    this.selectedFirstNotePosition = this.music.patterns.getDuration(this.patternId);

    for(var i = 0; i < notes.length; i++) {
      if(typeof notes[i].sel != 'undefined' && notes[i].sel) {
        if(notes[i].pos < this.selectedFirstNotePosition) {
          this.selectedFirstNotePosition = notes[i].pos;
        }
      }
    }
  },
  ...
  setHighlightNote: function(noteId) {
    if(this.highlightNoteId !== noteId) {
...
//          return this.mouseMoveHand(x, y, deltaX, deltaY);
        }
      }
    }
  ...
  mouseUpDrag: function(button, x, y) {
    ...
    this.music.history.startEntry('Move Notes');
//    this.music.history.startPatternChange(this.patternId);   

    // dont want to record the individual cut and paste actions
//    this.music.history.setEnabled(false);

    this.music.patterns.offsetSelectedNotes(this.patternId, this.dragOffsetX, this.dragOffsetY);

    this.music.history.endEntry();       
  ...
  mouseUpEffectsDraw: function(button, x, y) {
    ...
    if(this.mode == 'erase') {
      if(this.effectStart == this.effectEnd) {
        this.music.patterns.addParam(this.patternId, "effects", this.effectStart, 
                                    { "effect": 0, "value1": 0, "value2": 0 } );
  ...
  mouseUpErase: function(button, x, y) {
    ...
    var noteId = this.gridToNoteId(grid.x, grid.y);
    if(noteId !== false && this.currentNoteId === noteId) {
      // mouse up on note that mouse was down on
      this.music.patterns.eraseNote(this.patternId, noteId);
    }
    this.drawPattern();
  },
...
//    y = window.innerHeight - y - 1;
    ...
    if(this.inDrag) {
      ...
      if(this.resizeAmount != 0 && this.currentNoteId !== false) {

        if(this.resizeDirection == 'west') {

          var note = this.music.patterns.getNote(this.patternId, this.currentNoteId);
          var position = note.pos - this.resizeAmount;
          var duration = note.dur + this.resizeAmount;

          // only check overlap once set start and duration
          this.music.patterns.setNoteStart(this.patternId, this.currentNoteId, position, false);
          this.music.patterns.setNoteDuration(this.patternId, this.currentNoteId, duration);

          this.drawPattern();

        } else if(this.resizeDirection == 'east') {
          var noteDuration = this.music.patterns.getNoteDuration(this.patternId, this.currentNoteId) + this.resizeAmount;
...
//    this.selectedNotes = [];
  ...
  drawEffects: function() {
    ...
    if(this.patternId == -1 || this.patternId >= this.music.doc.data.patterns.length) {
      return;
    }
    this.rulerContext.beginPath();    
    ...
    for(var i = 0; i < patternDuration; i++) {
      ...
      if(param !== null) {
        var effect = param.values.effect;
        var duration = 1;
  ...
  drawOtherTrackNotes: function() {
    ...
    var globalPositionStart = this.music.getGlobalPosition(this.channel, this.trackIndex, 0);
    var globalPositionEnd = globalPositionStart + patternDuration;

    var tracks = this.music.doc.data.tracks;

    this.gridContext.globalAlpha = 0.15;

    for(var trackIndex = 0; trackIndex < tracks.length; trackIndex++) {
      if(trackIndex != this.channel) {
        for(var j = 0; j < tracks[trackIndex].patternInstances.length; j++) {
          var patternInstance = tracks[trackIndex].patternInstances[j];

          var patternId = patternInstance.patternId;
          var startPosition = patternInstance.position;
  
          var patternDuration = this.music.patterns.getDuration(patternId);  
          var endPosition = startPosition + patternDuration;

          var noteOffset = startPosition - globalPositionStart;

          
          var patternName = this.music.patterns.getName(patternId);    

          if( 
              (startPosition >= globalPositionStart && startPosition < globalPositionEnd)
              || (endPosition >= globalPositionStart && endPosition < globalPositionEnd)
          ) {
            // ok need to draw this pattern

            var note = null;
            var notes = this.music.patterns.getNotes(patternId);
            for(noteIndex = 0; noteIndex < notes.length; noteIndex++) {
              note = notes[noteIndex];
              var position = note.pos + noteOffset;
              var pitch = note.pit;
              var instrumentId = note.ins;
              var duration = note.dur;
              var selected = note.sel;
  ...
  drawNotes: function() {
    ...
    for(var i = 0; i < notes.length; i++) {
      ...
      if(note.noteId === this.highlightNoteId) {
        var noteColor = this.music.instruments.getHighlightColor(instrumentId);
        noteColor = ("000000" + noteColor.toString(16) ).substr(-6);        
      }
...
//      if( ( (selected || note.noteId == this.currentNoteId) && this.inDrag) 
//        || (note.noteId === this.currentNoteId && this.inResize)) {
      if(selected) {
...
//            instrumentColor = 'ff0000';
      ...
      if(!selected) {
        var noteColor = this.music.instruments.getBorderColor(instrumentId);
        noteColor = ("000000" + noteColor.toString(16) ).substr(-6);        

        this.gridContext.beginPath();    
        this.gridContext.strokeStyle = '#' + noteColor;//styles.music.noteOutline;
...
//      if( ( (selected || i == this.resizeNote) && this.inDrag) || (i === this.resizeNote && this.inResize)) {
  ...
  drawSelectedNotes: function() {
    if(this.patternId < 0 || this.patternId === false) {
      return;
    }
    var viewHeight = this.height - this.rulerHeight - this.effectsHeight - this.hScrollBarHeight;
...
//    }
    ...
    this.context.strokeStyle = styles.music.ghostNoteOutline;//'#ffffff';    
    ...
    for(var i = 0; i < notes.length; i++) {
      if(typeof notes[i].sel != 'undefined' && notes[i].sel) {
        var position = notes[i].pos;
        var pitch = notes[i].pit;
        var instrumentId = notes[i].ins;
        var duration = notes[i].dur;

        var noteColor = 'ff0000';

        if(instrumentId != false) {
          noteColor = this.music.instruments.getColor(instrumentId);
          noteColor = ("000000" + noteColor.toString(16) ).substr(-6);
        }
  ...
  drawCursorInfo: function() {
    this.cursorInfoPosition = this.cursor.position;
    this.cursorInfoPitch = this.cursor.pitch;

    if(this.cursorInfoPitch == -1) {
      return;
    }

    
    var viewHeight = this.height - this.rulerHeight - this.effectsHeight - this.hScrollBarHeight;
    var width = this.cellWidth / this.partsPerBeat * 1.4;
    var height = this.cellHeight; 
    var x = this.pianoRollWidth - Math.round(this.scrollX) + this.cursorInfoPosition * this.cellWidth;
    var y = 0;
    var cursorColor = '#222222';    
    var yOffset = -this.cellHeight ;

    var y = this.rulerHeight + this.effectsHeight + Math.round(this.scrollY) - (this.gridHeight * this.cellHeight - viewHeight)
    + (this.gridHeight * this.cellHeight) - ((this.cursorInfoPitch + 1) * this.cellHeight) + yOffset;
    ...
    var octave = Math.floor(this.cursorInfoPitch / 12);
    var noteName = notes[this.cursorInfoPitch % 12];
    var noteInfo = noteName + octave;

    if(this.highlightNoteId !== false) {
//      console.log(this.patternId + ', '+ this.resizeNote + ',' + this.currentNoteId);

      var note = this.music.patterns.getNote(this.patternId, this.highlightNoteId);
      if(note) {
        var instrumentId = note.ins;
        var instrument = this.music.instruments.getInstrument(instrumentId);
        
        noteInfo += ' ' + instrument.name;
      }
    }
    ...
    var textWidth = this.context.measureText(noteInfo).width;
    var width = textWidth + 4;
    this.context.globalAlpha = 0.8;
    ...
    this.context.fillText(noteInfo, x + 3, y + this.cellHeight - 3);


    // draw the box around the note

    
    this.context.strokeStyle = '#999999';//styles.music.cursorOutline;
  ...
  drawResizeNote: function() {
    ...
    if(this.currentNoteId === false || !this.inResize || this.inDrag) {
...
// scrolling if mouse moves outside
    if(time - this.lastScrollTime > 30) {
      ...
      if(this.xScrollSpeed != 0 ) {
        ...
        this.setXScroll(this.scrollX + this.xScrollSpeed * 4);
      } else if(this.yScrollSpeed != 0 ) {
        this.lastScrollTime = time;
        this.setYScroll(this.scrollY - this.yScrollSpeed * 4);
      }
    }
